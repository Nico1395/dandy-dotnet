...
public sealed class EventStoreTests : IClassFixture<DefaultFixture>
{
    ...
    public async Task GetStream_WithVersionAndTimestampFilters_ShouldApplyInclusiveBounds()
    {
        ...
        using (scope)
        {
            ...
            var timestampFiltered = await eventStore.GetStreamAsync(streamId, null, null, middle, null, CancellationToken.None);
            ...
            Assert.Empty(await eventStore.GetStreamAsync(streamId, null, 0, middle.AddDays(1), null, CancellationToken.None));
        }
    }
